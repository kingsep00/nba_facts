...
class ActivityUpComingMatches: AppCompatActivity() {
            ...
            call.enqueue(object : Callback<ListPostGames> {
                override fun onResponse(
                    ...
                    if (response.isSuccessful) {
                        ...
                        var objTeams = Gson().fromJson(json, Array<PostGames>::class.java).toList()
                        ...
                        objTeams = objTeams.filter { it.homeTeamScore!! == 0 }

                        objTeams = objTeams.sortedWith { a, b ->
                            val dateA = LocalDate.parse(a.date?.substring(0,10))
                            val dateB = LocalDate.parse(b.date?.substring(0,10))
                        ...
                        uiThread {
                            objTeams.forEach {
                                ...
                                Log.d("UPCOMING FOREACH DATA","post visitor ${it.visitorTeam?.abbrevation}  ${it.visitorTeamScore} - ${it.homeTeamScore} ${it.homeTeam?.abbrevation}")
                                Log.d("UPCOMING FOREACH DATA","post visitor ${it.date} post visitorScore ${it.visitorTeamScore} post homeTeamscore ${it.homeTeamScore} ")
                        ...
                        mRecyclerView.apply {
                            layoutManager = LinearLayoutManager(this@ActivityUpComingMatches)

                            adapter = PostAdapterUpComingGames(objTeams!!)
                        }
                    }
                }
